...
# ==========================================
# 1. SETUP THE APP CONFIGURATION
# ==========================================
st.set_page_config(page_title="Lizard Sampling Simulation", layout="wide")

st.title("🦎 Island Lizard Weight Study")
st.markdown("""
Welcome to our study of lizards on Osborne Island. 

Your goal is to estimate the true average weight of the lizard population on this island. 

However, there are four different types of lizards, each of which has different average sizes due to habitat:
1. Beach Lizard (mean = 50, SD = 5, N  =4000)
2. Jungle Lizard (mean= 120, SD = 15, N = 4000)
3. Wetlands Lizard (mean = 220, SD = 30, N = 1500)
4. Cave Lizard (mean = 350, SD = 60, N = 500)

Note also that Cave and Wetland lizards can be particularly difficult to find and catch.


**If you want to create stratified sampling, configure your sampling strategy in the sidebar.**
""")

# ==========================================
# 2. GENERATE POPULATION (Cached)
# ==========================================
# This function runs once and saves the result so the population 
# doesn't change every time you click a button.
@st.cache_data
def get_population():
    np.random.seed(101) 
    groups = {
...
if run_btn:
    ...
    # --- SIMPLE LOGIC ---
        ...
        if enable_bias:
            st.warning("⚠️ Field Conditions Active: Heavy lizards are harder to find!")
    ...
    if not sample_df.empty:
        ...
        # Map colors loosely to habitat index for consistency
        ax2.bar(counts.index, counts.values, color='teal')
